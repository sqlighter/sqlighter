...
export class Unit {
  ...
  public static async updateUnits() {
    const unitsPath = resolve("./contents/units.json")
    try {
...
const _unitsJson = require("../contents/units.json")
...
const _unitFuse: Fuse<any> = new Fuse<any>(
  Unit.getUnits().map((unit: any) => {
    return {
      ...
      conversions: unit.metadata?.conversions && Object.keys(unit.metadata?.conversions),
    }
  }),
  {
