...
describe("databasetreeview.tsx", () => {
  test("getTrees (chinook.db)", async () => {
    ...
    expect(rootTitles).toBe("Tables, Views, Triggers")

    const tables = tree.children[0]
    expect(tables.id).toBe("main/tables")
    ...
    expect(albums.id).toBe("main/tables/albums")
    ...
    expect(albumsColumns.id).toBe("main/tables/albums/columns")
    ...
    expect(albumIdColumn.id).toBe("main/tables/albums/columns/AlbumId")
    ...
    expect(titleColumn.id).toBe("main/tables/albums/columns/Title")
  ...
  test("getTree (test.db)", async () => {
    ...
    expect(views.id).toBe("main/views")
    ...
    const triggers = tree.children[2]
    expect(triggers.id).toBe("main/triggers")
