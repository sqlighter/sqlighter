			...
			if (biomarker.unit?.conversions) {
				Object.keys(biomarker.unit.conversions).forEach((unit) => {
					if (biomarker.metadata.conversions?.[unit]) {
						throw new Error(`Biomarker ${biomarker.id} - Unit ${biomarker.unit?.id} should not contain a conversion to ${unit}`);
					}
				});
			}
		}
		...
		for (const unit of Unit.getUnits()) {
			if (unit.conversions) {
				for (const conversion of Object.keys(unit.conversions)) {
					if ((from.indexOf(unit.id) != -1 && to.indexOf(conversion) != -1) || (from.indexOf(conversion) != -1 && to.indexOf(unit.id) != -1)) {
